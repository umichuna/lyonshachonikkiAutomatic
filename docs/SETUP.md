...
- **発行されたURL・履歴の「記事を開く」が404になる** → `GITHUB_REPO` の大文字小文字がリポジトリ名と完全に一致しているか確認してください(例: `lyonshachonikkiAutomatic`。小文字の`lyonshachonikkiautomatic`だと404になります)。GitHub PagesはURLのリポジトリ名部分の大文字小文字を区別するため、GitHubへのコミット自体はcase-insensitiveで成功しても、公開URLだけ404になることがあります。修正したら再デプロイし、以後の投稿・修正でURLが正しく生成されるか確認してください
